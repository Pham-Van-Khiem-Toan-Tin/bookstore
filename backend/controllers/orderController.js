...
const addOrderItems = asyncHandler(async (req, res) => {
  ...
  if (orderItems && orderItems.length === 0) {
    ...
    const bookListIds = items.map((item) => item.book);
    const bookListCountInStock = await bookModel.find({ _id: { $in: bookListIds } });
    const checkOutOfStock = items.every((item) => item.qty <= bookListCountInStock.find((book) => book._id.toString() === item.book).countInStock);
    if (!checkOutOfStock) {
      res.status(500);
